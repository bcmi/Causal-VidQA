...
class MemoryRamTwoStreamModule2(nn.Module):

    def __init__(self, input_size, hidden_size=512, memory_bank_size=100, device=None):
        """Set the hyper-parameters and build the layers."""
        super(MemoryRamTwoStreamModule2, self).__init__()
    ...
    def forward(self, hidden_out_a, hidden_out_m, nImg):
        ...
        memory_ram = torch.FloatTensor(hidden_out_a.shape[0], self.memory_bank_size, self.hidden_size).to(self.device)
        memory_ram.fill_(0)
        
        h_t_a = torch.zeros(hidden_out_a.shape[0], 1, self.hidden_size).to(self.device)
        h_t_m = torch.zeros(hidden_out_a.shape[0], 1, self.hidden_size).to(self.device)
        h_t = torch.zeros(hidden_out_a.shape[0], 1, self.hidden_size).to(self.device)

        hiddens = torch.FloatTensor(hidden_out_a.shape[0], nImg, self.hidden_size).to(self.device)
        ...
        for t in range(nImg):
            ...
            ############# read ############
            x_h_t_am = torch.cat([h_t_a,h_t_m,h_t],dim=2)
            ar = torch.softmax(self.read_prob( x_h_t_am ),dim=2)  # read prob from memories
            r = torch.matmul(ar,memory_ram)  # read vector


            ######### h_t #########
            # Eq (17)
            f_0 = torch.cat([r, h_t],dim=2)
            f_a = torch.cat([x_t_a, r, h_t_a],dim=2)
            f_m = torch.cat([x_t_m, r, h_t_m],dim=2)
            ...
            # write probability of [keep, write appearance, write motion]
            aw = torch.softmax(self.write_prob( x_h_t_am ),dim=2)  # write prob to memories
            x_h_ta = torch.cat([h_t_a,x_t_a],dim=2)
            x_h_tm = torch.cat([h_t_m,x_t_m],dim=2)
            ...
            aw_a = torch.softmax(self.write_prob_a( x_h_ta ),dim=2)  # write prob to memories
            aw_m = torch.softmax(self.write_prob_m( x_h_tm ),dim=2)  # write prob to memories


            aw_a = aw_a.view(hidden_out_a.shape[0], self.memory_bank_size,1)
            aw_m = aw_m.view(hidden_out_a.shape[0], self.memory_bank_size,1)
            
            memory_ram = aw[:, 0,0].unsqueeze(1).unsqueeze(2) * memory_ram + aw[:, 0,1].unsqueeze(1).unsqueeze(2) * aw_a * c_t_a + aw[:, 0,2].unsqueeze(1).unsqueeze(2) * aw_m * c_t_m
            ...
            hiddens[:, t,:] = h_t.squeeze()

        
        return hiddens

class MemoryRamModule2(nn.Module):

    def __init__(self, input_size=1024, hidden_size=512, memory_bank_size=100, device=None):
        """Set the hyper-parameters and build the layers."""
        super(MemoryRamModule2, self).__init__()
    ...
    def forward(self, hidden_frames, nImg):
        ...
        memory_ram = torch.FloatTensor(hidden_frames.shape[0], self.memory_bank_size, self.hidden_size).to(self.device)
        memory_ram.fill_(0)
        
        h_t = torch.zeros(hidden_frames.shape[0], 1, self.hidden_size).to(self.device)
        
        hiddens = torch.FloatTensor(hidden_frames.shape[0], nImg, self.hidden_size).to(self.device)
                
        for t in range(nImg):
            x_t = hidden_frames[:, t:t+1,:]

            x_h_t = torch.cat([x_t,h_t],dim=2)
                        
            ############# read ############
            ar = torch.softmax(self.read_prob( x_h_t ),dim=2)  # read prob from memories
            go = torch.sigmoid(self.read_gate( x_h_t ))  # read gate
            r = go * torch.matmul(ar,memory_ram)  # read vector
            ...
            ############# write ############            
            c_t = F.relu( self.hidden_to_content(x_h_t) )  # Eq(15), content vector
            aw = torch.softmax(self.write_prob( x_h_t ),dim=2)  # write prob to memories
            aw = aw.view(-1, self.memory_bank_size,1)
            gw = torch.sigmoid(self.write_gate( x_h_t ))  # write gate
            #print gw.size(),aw.size(),c_t.size(),memory_ram.size()
            memory_ram = gw * aw * c_t + (1.0-aw) * memory_ram # Eq(16)
...
class MMModule2(nn.Module):
    def __init__(self, dim, input_drop_p, device):
        """Set the hyper-parameters and build the layers."""
        super(MMModule2, self).__init__()
        self.hidden_size = dim
        self.lstm_mm_1 = nn.LSTMCell(dim, dim)
        self.lstm_mm_2 = nn.LSTMCell(dim, dim)
        self.hidden_encoder_1 = nn.Linear(dim * 2, dim)
    ...
    def init_hiddens(self, bs):
        s_t = torch.zeros(bs, self.hidden_size).to(self.device)
        s_t2 = torch.zeros(bs, self.hidden_size).to(self.device)
        c_t = torch.zeros(bs, self.hidden_size).to(self.device)
        c_t2 = torch.zeros(bs, self.hidden_size).to(self.device)
        return s_t, s_t2, c_t, c_t2
    ...
    def forward(self, svt_tmp, memory_ram_vid, memory_ram_txt, loop=3):
        ...
        """
        bs = svt_tmp.shape[0]
        sm_q1, sm_q2, cm_q1, cm_q2 = self.init_hiddens(bs)
        mm_oo = self.dropout(torch.tanh(self.hidden_encoder_1(svt_tmp)))
